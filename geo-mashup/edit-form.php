...
function geo_mashup_edit_form( $object_name, $object_id, $ui_manager = '' ) {
	...
	if ( 'post' == $object_name) {
		...
		//if ( get_option( 'trail_story_options[enable_kmls]' ) == True ) {
			//$kml_url = get_option( 'trail_story_options[custom_kml_layers]' );
		//}

		//$enable = 'enable_kmls';
		//$key = 'custom_kml_layers';
		//$options = get_option( 'trail_story_options' );
		//$kml_enabled = $options[$enable];
		//$kml_layers = $options[$key];

		$kml_urls = GeoMashup::get_kml_attachment_urls( $object_id );
		if (count($kml_urls)>0) {
			$kml_url = array_pop($kml_urls);
		} else {
			$kml_url = 'http://www.iceagetrail.org/wp-content/uploads/2015/12/iat-connector.kml';
		}
